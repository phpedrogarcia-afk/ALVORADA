...
**Estado:** CONDITIONAL — G1.1 PASS; G1.2 `E0_PASS`; G1.3 / MISSÃO 05R-C `LAB_DISCOVERY_CONDITIONAL_REMEDIATION_PROBE_NOT_RUN`. Repo público, Actions e snapshot vinculados; o discovery observou capacidade parcial, mas nenhum precheck, envelope Android ou físico foi validado.
...
MISSÃO 05R-C está **`LAB_DISCOVERY_CONDITIONAL_REMEDIATION_PROBE_NOT_RUN`**, custo USD 0 e zero execuções E1. O próximo passo autorizado é somente a remediation probe manual: corrigir efemeramente o acesso a KVM se o `sudo` não-interativo e `setfacl` já existirem, e resolver tooling Android apenas nos roots esperados. Não instalar API 36, system image ou AVD; não iniciar guest, P1–P10, Wave 1, E2, G2, app de produto, UI final, stack de produção ou suporte físico.
