...
/** Generates a small deterministic PCM marker for future external detection. */
public final class AudioMarker {
    ...
    public static String sha256() {
        return AlarmModel.sha256Hex(wavBytes());
    }
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            System.err.println("usage: AudioMarker <output.wav>");
            System.exit(64);
        }
        Path output = Path.of(args[0]);
        write(output);
        System.out.println("audio_marker_path=" + output.toAbsolutePath());
        System.out.println("audio_marker_size=" + wavBytes().length);
        System.out.println("audio_marker_sha256=" + sha256());
    }
