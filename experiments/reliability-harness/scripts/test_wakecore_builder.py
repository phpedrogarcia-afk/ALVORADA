...
from wakecore_builder import (
    PACKAGE_NAME,
    CONTRACT_WAKECORE_BUILD,
    WakeCoreBuilder,
    WakeCoreBuildError,
)


class TestWakeCoreBuilder(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = tempfile.TemporaryDirectory()
        self.root = Path(self.temp_dir.name)
        self.sdk_root = self.root / "sdk"
        self.wakecore_dir = self.root / "wakecore"
        self.build_dir = self.root / "build"

        self.sdk_root.mkdir(parents=True, exist_ok=True)
        self.wakecore_dir.mkdir(parents=True, exist_ok=True)
        ...
        (self.wakecore_dir / "AndroidManifest.xml").write_text("<manifest></manifest>")
        src_pkg = self.wakecore_dir / "src" / "org" / "alvorada" / "reliability" / "wakecore"
        src_pkg.mkdir(parents=True, exist_ok=True)
        (src_pkg / "WakeConstants.java").write_text("package org.alvorada.reliability.wakecore; class WakeConstants {}")

        self.builder = WakeCoreBuilder(
            sdk_root=self.sdk_root,
            wakecore_dir=self.wakecore_dir,
    ...
    def test_build_apk_end_to_end_mocked(self, mock_run: MagicMock) -> None:
        def side_effect(cmd, *args, **kwargs):
            ...
            if "aapt2 link" in cmd_str:
                ...
                obj = self.build_dir / "obj" / "WakeConstants.class"
                ...
                out_apk = self.build_dir / "alvorada-wakecore.apk"
        ...
        verified = self.builder.verify_apk(apk)
